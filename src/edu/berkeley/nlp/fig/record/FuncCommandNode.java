    ...
    public RecordNode exec() {
      ...
      if(name.equals("define")) {
        ...
        if(name.equals("source")) {
          ...
          // Arguments: none
          CarefulFullStatFig fig = new CarefulFullStatFig();
          for(RecordNode node : result.getChildren())
            fig.add(node.getDoubleValue());
          newFullResult.addChild(new LeafRecordNode("n", ""+fig.size()));
          newFullResult.addChild(new LeafRecordNode("min", ""+fig.getMin()));
          newFullResult.addChild(new LeafRecordNode("max", ""+fig.getMax()));
          newFullResult.addChild(new LeafRecordNode("mean", ""+fig.mean()));
          newFullResult.addChild(new LeafRecordNode("stddev", ""+fig.stddev()));
          newFullResult.addChild(new LeafRecordNode("sum", ""+fig.total()));
          if(fig.numNaN() > 0)    newFullResult.addChild(new LeafRecordNode("numNaN", ""+fig.numNaN()));
          if(fig.numPosInf() > 0) newFullResult.addChild(new LeafRecordNode("numPosInf", ""+fig.numPosInf()));
          if(fig.numNegInf() > 0) newFullResult.addChild(new LeafRecordNode("numNegInf", ""+fig.numNegInf()));
        }
          ...
          for(double evalx : evalxs) { // For each evaluation point x
            ...
            if(fig.size() == R) {
              if(mode.equals("none"))
                ...
                newResult.addChild(new LeafRecordNode(key, evalx + " " + fig.mean() + " " + fig.getMin() + " " + fig.getMax()));
              else
