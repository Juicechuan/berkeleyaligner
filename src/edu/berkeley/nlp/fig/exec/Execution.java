package edu.berkeley.nlp.fig.exec;
...
import static edu.berkeley.nlp.fig.basic.LogInfo.stderr;
...
import edu.berkeley.nlp.fig.basic.CharEncUtils;
import edu.berkeley.nlp.fig.basic.ClassInitializer;
...
import edu.berkeley.nlp.fig.basic.OptionsParser;
import edu.berkeley.nlp.fig.basic.OrderedStringMap;
import edu.berkeley.nlp.fig.basic.StopWatchSet;
import edu.berkeley.nlp.fig.basic.StrUtils;
import edu.berkeley.nlp.fig.basic.SysInfoUtils;
import edu.berkeley.nlp.fig.basic.Utils;
import edu.berkeley.nlp.fig.record.Record;
...
public class Execution {
  ...
  @Option(gloss="Use Scala to run rather than Java")
    public static boolean thunkUseScala = false;
	...
	private static void initializeJars(boolean inVirtualExecDir)
{
  ...
  public static void runWithObjArray(String[] args, Object[] objects)
  {
    ...
    if(makeThunk) {
      ...
      if(thunkUseScala) { // Run as Scala
        cmd.add("env");
        if(thunkJavaOpts != null) cmd.add("'JAVA_OPTS="+thunkJavaOpts+"'");
        cmd.add("scala");
      }
      else { // Run of Java
        cmd.add("java");
